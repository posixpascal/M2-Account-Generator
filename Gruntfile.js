...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/M2 Account Generator.zip'
        },
        files: [
          {
            expand: true,
        ...
        files: [
          {
            ...
            dest: '<%= config.dist %>/M2 Account Generator.app'
          }
        ]
      },
      zipToApp: {
  ...
  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/M2 Account Generator.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/M2 Account Generator.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/M2 Account Generator.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/M2 Account Generator.app/Contents/MacOS/node-webkit', '555');
  });
  ...
  grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
    ...
    var childProcess = require('child_process');
    var exec = childProcess.exec;
    exec('copy /b tmp\\nw.exe+tmp\\app.nw tmp\\m2-account-generator.exe && del tmp\\app.nw tmp\\nw.exe && echo.>tmp\\ready', function (error, stdout, stderr) {
      console.log(stderr, stdout, error);
    });
